...
describe("Hero.jsx - Frontend Unit Tests", () => {
  ...
  it("UT-01-004: Skips sending search for empty/whitespace input", () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/type a city/i), {
      target: { value: "   " },
    });
  ...
  it("UT-01-005: Suggestions update when worker sends results", async () => {
    ...
    workerInstance.onmessage({
      data: { results: [{ term: "London", uid: "A1", type: "city" }] },
    });
  ...
  it("UT-03-002/004: Decrement buttons disabled at min limits", () => {
    render(
      <MemoryRouter>
        <Hero initialRooms={1} initialGuestsPerRoom={1} />
      </MemoryRouter>
    );
  ...
  it("UT-03-006: Error if totalGuests < rooms", async () => {
    render(
      <MemoryRouter>
        <Hero
          initialRooms={3}
          initialGuestsPerRoom={0}
          initialDestination="Rome, Italy"
        />
      </MemoryRouter>
    );
    fireEvent.click(screen.getByText(/^search$/i));
    expect(
      await screen.findByText(/each room must have at least 1 guest/i)
    ).toBeInTheDocument();
  });
  ...
  it("UT-03-007: Error if totalGuests > maxTotalGuests", async () => {
    ...
    fireEvent.click(screen.getByText(/^search$/i));
    expect(
      await screen.findByText(/total guests cannot exceed 10/i)
    ).toBeInTheDocument();
  });
  ...
  it("UT-04-001: Shows destination error if empty after submit", async () => {
    ...
    fireEvent.click(screen.getByText(/^search$/i));
    expect(
      await screen.findByText(/please choose a destination from the list\./i)
    ).toBeInTheDocument();
  });
  ...
  it("UT-04-002: Shows destination error if not in list", async () => {
    render(
      <MemoryRouter>
        <Hero initialDestination="MadeUpPlace" />
      </MemoryRouter>
    );
    fireEvent.click(screen.getByText(/^search$/i));
    expect(
  ...
  it("UT-04-003a: Shows error if check-in date missing", async () => {
    render(
      <MemoryRouter>
        <Hero initialDestination="Rome, Italy" />
      </MemoryRouter>
    );
    fireEvent.click(screen.getByText(/^search$/i));
    expect(
      await screen.findByText(/please select a check-in date\./i)
    ).toBeInTheDocument();
  });
  ...
  it("UT-04-003b: Shows error if check-in date before min", async () => {
    ...
    fireEvent.click(screen.getByText(/^search$/i));
    expect(
      await screen.findByText(/check-in date can't be before/i)
    ).toBeInTheDocument();
  });
  ...
  it("UT-04-004a: Shows error if check-out date missing", async () => {
    render(
      <MemoryRouter>
        <Hero initialDestination="Rome, Italy" initialCheckIn="2025-12-01" />
      </MemoryRouter>
    );
    fireEvent.click(screen.getByText(/^search$/i));
    expect(
      await screen.findByText(/please select a check-out date\./i)
    ).toBeInTheDocument();
  });
  ...
  it("UT-04-004b: Shows error if check-out date ≤ check-in date", async () => {
    ...
    expect(
      await screen.findByText(
        /check-out date cannot be before or equal to check-in date\./i
      )
    ).toBeInTheDocument();
  });
});
